...
# --- AGENTS.md -------------------------------------------------------------
if [ -f "AGENTS.md" ]; then
echo "AGENTS.md already exists (leaving unchanged)."
else
cat > AGENTS.md <<EOF
# Codex Operating Guide

This repository has been minimally bootstrapped. Codex should verify source,
tests, configs, and current git state before trusting generated planning docs.

## First Five Minutes
1. Run \`git status --short --branch\`.
2. Inspect changed files before editing.
3. Use \`rg --files\` and targeted \`rg\` searches to find current source truth.
4. Read relevant source, tests, configs, Dockerfiles, and package manifests.
5. Treat planning docs as historical until verified.
EOF
...
git add AGENT_CONTRACT.md PROJECT_POLICY.yaml AGENTS.md .gitignore docs adr 2>/dev/null || true
