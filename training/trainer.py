...
from .env import DEFAULT_SHIPS, BattleshipEnv
...
def run_training(config: TrainConfig) -> Dict[str, str]:
    ...
    learner = QLearner(
        env=env, epsilon=config.epsilon, epsilon_decay=config.epsilon_decay, lr=config.lr
    )
    print(
    ...
    print(
        f"[train] wrote artifact={artifact_path} manifest={manifest_path}",
        flush=True,
        file=sys.stderr,
    )
    return {
